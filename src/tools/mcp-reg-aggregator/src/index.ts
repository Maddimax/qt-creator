...
async function condenseServer(srv: ServerResponse): Promise<CondensedServer> {
  const { server } = srv;
  ...
  return {
    ...
    icons,
    packages: (server.packages ?? []).map(condensedPackage),
...
async function uploadToS3(payload: string): Promise<void> {
  ...
  await client.send(
    new PutObjectCommand({
      Bucket: bucket,
      Key: "mcp-registry.json",
      Body: payload,
      ContentType: "application/json",
    })
  );
  console.log(`Uploaded mcp-registry.json to s3://${bucket}/mcp-registry.json`);
}
...
async function main(): Promise<void> {
  ...
  // Optional array fields that should be omitted when empty
  const optionalArrayKeys = new Set(["headers", "icons", "sizes"]);

  const json = JSON.stringify(output, (key, value) => {
    if (value === null) return undefined;
    if (Array.isArray(value) && value.length === 0 && optionalArrayKeys.has(key)) return undefined;
    return value;
  }, 2);
