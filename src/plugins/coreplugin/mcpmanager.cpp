...
static std::optional<McpRegistry::McpRegistry> s_mcpRegistry;

static const char MCP_REGISTRY_URL[] = "http://localhost:8080/mcp-registry.json";
//= "https://cdn.agentclientprotocol.com/registry/v1/latest/mcp-registry.json";

static void fetchMcpRegistry(std::function<void()> onDone = {})
{
    if (s_mcpRegistry) {
        if (onDone)
            onDone();
    ...
    const auto fetchDone = [onDone](const QNetworkReplyWrapper &wrapper, DoneWith doneWith) {
        ...
        if (error.error != QJsonParseError::NoError) {
            qWarning() << "Failed to parse MCP registry JSON:" << error.errorString();
            return;
        }

        auto registry = McpRegistry::fromJson<McpRegistry::McpRegistry>(doc.object());
        if (!registry) {
            qWarning() << "Failed to parse MCP registry:" << registry.error();
            return;
        }
        s_mcpRegistry = std::move(*registry);
        if (onDone)
            onDone();
    };

    GlobalTaskTree::start({QtTaskTree::QNetworkReplyWrapperTask(setupFetch, fetchDone)});
}
...
static const McpRegistry::Icon *bestIconForTheme(const QList<McpRegistry::Icon> &icons)
{
...
static FilePath mcpIconCachePath(const McpRegistry::Icon &icon)
{
    const QByteArray hash
        = QCryptographicHash::hash(icon.url().toUtf8(), QCryptographicHash::Sha1);
...
static void fetchMcpIconAsync(
    const McpRegistry::Server &server, std::function<void(const QIcon &)> onDone)
{
    if (!onDone)
        return;
    ...
    const FilePath destPath = mcpIconCachePath(*icon);
    if (destPath.isFile()) {
        onDone(QIcon(destPath.toFSPathString()));
        return;
    }
    ...
    const auto fetchDone = [onDone, destPath](const FileStreamer &task,
                                                    QtTaskTree::DoneWith doneWith) {
...
public:
        ...
        : QAbstractListModel(parent)
    {
        if (!s_mcpRegistry)
            return;
        for (const auto &server : s_mcpRegistry->servers()) {
            if (server.status() != McpRegistry::Server::Status::deleted)
    ...
    QVariant data(const QModelIndex &index, int role) const override
    {
        ...
        const McpRegistry::Server &server = *m_servers.at(index.row());
...
private:
    QIcon iconForRow(int row) const
    {
        ...
        const McpRegistry::Server &server = *m_servers.at(row);
        ...
        fetchMcpIconAsync(server, [this, row](const QIcon &icon) {
            if (!icon.isNull()) {
                m_icons[row] = icon;
    ...
    static QStringList typeTags(const McpRegistry::Server &server)
    {
        ...
        if (!server.remotes().isEmpty() && !tags.contains(QString("remote")))
            tags.append(QString("remote"));
        return tags;
    }

    QList<const McpRegistry::Server *> m_servers;
...
public:
        ...
        : QDialog(parent)
    {
        ...
        m_model = new McpRegistryModel(this);
    ...
    std::optional<McpRegistry::Server> selectedServer() const
    {
        ...
        const auto *server = m_model->serverAt(srcIdx.row());
        if (!server)
            return {};
        return *server;
    }
...
private:
    ...
    McpRegistryModel *m_model = nullptr;
...
static QStringList serializeArguments(const QList<McpRegistry::Argument> &arguments)
{
    ...
    for (const auto &arg : arguments) {
        const std::optional<QString> argValue = arg.value().has_value()      ? arg.value()
                                                : arg.default_().has_value() ? arg.default_()
                                                : arg.value_hint().has_value()
                                                    ? arg.value_hint()
...
static void applyPackageToAspect(McpServerAspect &aspect, const McpRegistry::Package &pkg)
{
    ...
    if (pkg.transport_type() == McpRegistry::Package::Transport_type::stdio) {
        ...
        const QStringList runtimeArgs = serializeArguments(pkg.runtime_arguments());
        const QStringList packageArgs = serializeArguments(pkg.package_arguments());
