...
Utils::Result<Package> fromJson<Package>(const QJsonValue &val) {
    ...
    if (!obj.contains("runtime_arguments"))
        co_return Utils::ResultError("Missing required field: runtime_arguments");
    if (!obj.contains("package_arguments"))
        co_return Utils::ResultError("Missing required field: package_arguments");
    if (!obj.contains("env_vars"))
        co_return Utils::ResultError("Missing required field: env_vars");
    Package result;
    ...
    if (obj.contains("runtime_arguments") && obj["runtime_arguments"].isArray()) {
        ...
        for (const QJsonValue &v : arr) {
            result._runtime_arguments.append(co_await fromJson<Argument>(v));
        }
    }
    if (obj.contains("package_arguments") && obj["package_arguments"].isArray()) {
        const QJsonArray arr = obj["package_arguments"].toArray();
        for (const QJsonValue &v : arr) {
            result._package_arguments.append(co_await fromJson<Argument>(v));
        }
    }
    ...
    if (obj.contains("env_vars") && obj["env_vars"].isArray()) {
        ...
        for (const QJsonValue &v : arr) {
            result._env_vars.append(co_await fromJson<KeyValueInput>(v));
        }
    }
    co_return result;
}
...
Utils::Result<Server> fromJson<Server>(const QJsonValue &val) {
    ...
    if (!obj.contains("icons"))
        co_return Utils::ResultError("Missing required field: icons");
    if (!obj.contains("packages"))
        co_return Utils::ResultError("Missing required field: packages");
    if (!obj.contains("remotes"))
        co_return Utils::ResultError("Missing required field: remotes");
    Server result;
    result._name = obj.value("name").toString();
